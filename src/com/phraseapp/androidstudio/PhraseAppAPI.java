...
public class PhraseAppAPI {
    ...
    public static final String PHRASE_AUTH_TOKEN_VALUE = PropertiesComponent.getInstance().getValue(PHRASE_AUTH_TOKEN_KEY);

    public static final String PHRASE_UPDATE_TRANSLATIONS_KEY = "PHRASE_UPDATE_TRANSLATIONS_KEY";
    ...
    private NameValuePair[] addAuthToken(){
        return (new NameValuePair[]{
                    new NameValuePair("auth_token", PHRASE_AUTH_TOKEN_VALUE)}
        );
    }
     ...
     */
    public boolean uploadLocaleFile(String locale, String content) {
        PostMethod method = new PostMethod(PHRASE_API_BASEURL + "file_imports");
        method.getParams().setContentCharset("UTF-8");
        method.setParameter("auth_token", PHRASE_AUTH_TOKEN_VALUE);
        method.setParameter("file_import[locale_code]", locale);
        if(PropertiesComponent.getInstance().getValue(PhraseAppAPI.PHRASE_UPDATE_TRANSLATIONS_KEY).equals("1")){
